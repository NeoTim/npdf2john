...
%PDF::Resources = (
    ...
    XObject => {
        Condition => '$self->Options("ExtractEmbedded")',
        SubDirectory => { TagTable => 'PDF::XObject' },
    },
    Properties => {
        Condition => '$self->Options("ExtractEmbedded")',
        SubDirectory => { TagTable => 'PDF::Properties' },
    },
);
...
%PDF::AIPrivate = (
    ...
    AIPrivateData => {
        ...
        SubDirectory => { TagTable => 'PostScript::Main' },
    },
    AIPDFPrivateData => {
        JoinStreams => 1,   # join streams from numbered tags and process as one
        SubDirectory => { TagTable => 'PostScript::Main' },
...
%PDF::ImageResources = (
    _stream => {
        SubDirectory => { TagTable => 'Photoshop::Main' },
    },
);
...
sub CipherUpdate($)
{
    ...
    while ($pos + 16 <= length($dat)) {
        substr($dat,$pos,16) = AES::Cipher(substr($dat,$pos,16));
        $pos += 16;
    }
    if ($pos < length $dat) {
...
sub DecryptInit($$$)
{
    ...
    if ($ver == 1 or $ver == 2) {
        ...
        if ($ver == 5) {
            ...
            require AES;   # will need this later
        }
    } else {
...
sub Crypt($$;$)
{
    ...
    if ($$cryptInfo{_aesv2} or $$cryptInfo{_aesv3}) {
        require AES;
        my $err = AES::Crypt($dataPt, $key, $encrypt);
    ...
    for (;;) {
        ...
        if ($verbose) {
            ...
            if ($isSubdir) {
                ...
                $tagInfo or $tagInfo = {
                    Name => $tag,
                    SubDirectory => { TagTable => 'PDF::Unknown' },
                };
            } else {
